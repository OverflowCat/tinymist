...
Before the release build, the job will install Node.js 24 and the pinned `ohos-binary-sign@1.0.0` npm package, then enable ARM64 emulation with the standard `tonistiigi/binfmt` image. After the build, it invokes the package's bundled `platform/openharmony-arm64/binary-sign-tool` dependency with `-selfSign 1` inside the `ghcr.io/hqzing/dockerharmony` OpenHarmony userland, mounting the workspace so the binary is signed in place. This avoids loading the wrapper's host-platform guard and supplies the musl loader and C++ runtime that the signer requires. The signed result is then used for both the standalone OHOS artifact and the VSIX payload. A missing tool, emulator, runtime, or signing error fails the job before upload.
...
- [The npm wrapper only supports OpenHarmony ARM64 hosts] -> Extract its bundled `binary-sign-tool` dependency and execute it in the author's OpenHarmony userland image under registered ARM64 emulation on the Ubuntu cross-compilation runner.
- [Docker image or emulation setup may be unavailable on a runner] -> Enable binfmt explicitly, keep the container invocation isolated to signing, and fail before upload if it cannot run; the target binary itself remains unmodified unless signing succeeds.
